...
def main():
    ...
    for f in glob.glob(csv_dir + "*.csv"):
        ...
        axes[2].set_ylabel("Left")
        axes[3].plot(idx, z, linewidth=2)
        axes[3].set_xlabel("Elapsed Time (s)", fontsize=15)
        axes[3].set_ylabel("Right" )
        axes[3].set_xlim(left=0, right=idx[-1])
        analysis_utils.add_user_marks(act, noact, fig=fig, ax=axes[2], vals=[500, -500])
        analysis_utils.add_user_marks(act, noact, fig=fig, ax=axes[3], vals=[500, -500])
