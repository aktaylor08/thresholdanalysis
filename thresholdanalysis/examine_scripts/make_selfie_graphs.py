...
def main():
    directory = '/Users/ataylor/Research/thesis_data/selfie/'
    if directory[-1] != '/':
        directory += '/'
    ...
    for f in glob.glob(csv_dir + "*.csv"):
        ...
        y = df['a_subject_pose__translation_y'].dropna().values
        z = df['a_subject_pose__translation_z'].dropna().values
        yaw = df['a_subject_pose__rotation_z'].dropna().values
        x = np.append(x, [x[-1]])
        z = np.append(z, [z[-1]])
        y = np.append(y, [y[-1]])
        ...
        axes[0].plot(idx,  y, linewidth=2)
        axes[0].set_ylabel("X  and Y Pos (m)", )

        axes[1].plot(idx, z, linewidth=2)
        axes[1].set_ylim(0,2)
        axes[1].set_ylabel("Height (m)")

        axes[2].plot(idx, yaw, linewidth=2)
        axes[2].set_ylabel("Yaw (rad)")
        axes[2].set_ylim(-.5,7)

        axes[0].set_xlim(left=0, right=idx[-1])
        axes[1].set_xlim(left=0, right=idx[-1])
        axes[2].set_xlim(left=0, right=idx[-1])
        ...
        analysis_utils.add_user_marks(act, noact, fig=fig, ax=axes[2])
