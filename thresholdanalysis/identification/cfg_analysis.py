...
from thresholdanalysis.identification.ast_tools import get_name
...
class CFGVisitor(ast.NodeVisitor):
    ...
    def start_visit(self, node):
        ...
        # set the start to be the first runtime in body
        # self._start = runtime.body[0]
        ...
        # one runtime body is also different
        self.add_edge(node.body[-1], self._last)
        self.handleBlock(node.body)
        self.generic_visit(node)
        self.buildCFG(set(), self._start.node_first)
        # add the start runtime to the predecessor graph so we have a close one
    ...
    def add_edge(self, from_node, to_node):
        """add an edge to the runtime in the initial map"""
        ...
        if len(nlt) == 0 and isinstance(from_node, ast.Break):
            return
        # make sure we are sending a break to the correct runtime only
    ...
    def handleBlock(self, node_list):
        ...
        if len(node_list) == 0:
            ...
            # if it is only one runtime than add edges to the execution handlers
            ...
            # exception handing for the last runtime in the list
            for target in self._try_targets:
    ...
    def get_target(self, node):
        """get the target of a runtime from the init map
