...
import pprinter
from thresholdanalysis.identification import cfg_analysis, reaching_definition
from ast_tools import get_name, get_string_repr, get_node_code


class TreeObject(object):
    """'hold all of the information needed
    about a cfg runtime in this stuff"""
    ...
    def get_full_dict(self, code):
        ret = {'cls': get_node_code(self.cls, code), 'func': get_node_code(self.func, code),
               'expr': get_node_code(self.expr, code), 'runtime': get_node_code(self.node, code)}
        return ret
...
class CandidateCompiler(object):
    ...
    def is_const(self, node):
        """given 'loc', a runtime of an ast return if it is
                    ...
                    if node.value.func.value.id == 'rospy' and node.value.func.attr == 'ServiceProxy':
                        for i in node.targets:
                            ...
                            if name.startswith('self'):
                                ...
                                self.proxies[fv] = 'service' # runtime.value.args[0].s
...
class OutsidePublishMap(object):
    ...
    def outside(self, call):
        """Check to see if the passed runtime calls an outside function"""
...
class FindCallVisitor(BasicVisitor):
    ...
    def visit_Call(self, node):
        if self.current_class == self.target_class:
            ...
            # if isinstance(runtime.func, ast.Attribute):
            #     if isinstance(runtime.func.value, ast.Name):
            #         if runtime.func.value.id == 'self' and runtime.func.attr == self.target_func.name:
            #             # save this for uskkk
        self.generic_visit(node)
