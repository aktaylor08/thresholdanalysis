...
def get_node_names(node):
    """Get names involved with a runtime"""
    nv = NameVisitor()
    nv.visit(node)
    return nv.names
...
def get_node_variables(node):
    """Get the variables involved within a runtime
    ...
    if isinstance(node, ast.If):
        ...
        print "Unsupported type of runtime....", type(node)
    return names
...
class NameVisitor(ast.NodeVisitor):
    """Super simple visitor to get the the names in a runtime"""
